...
class GameScene: SKScene {
    ...
    var score: Int = 0 {
        didSet {
            // Updates score label when score changes
    ...
    var popupTime = 0.9 // time between spawn
    var sequence: [SequenceType]! // Array of types
    var sequencePosition = 0
    var chainDelay = 3.0 // how long before create new one
    var nextSequenceQueued = true // all enemies are destroyed
    
    var gameEnded = false
    
    override func didMove(to view: SKView) {
        
        let background = SKSpriteNode(imageNamed: "sliceBackground.jpg")
        ...
        sequence = [.oneNoBomb, .twoWithOneBomb, .twoWithOneBomb, .three, .one, .chain]
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        // Remove all existing points in activeSlice points
        activeSlicePoints.removeAll(keepingCapacity: true)
        
        // Get touch location and add to slice points
        ...
        redrawActiveSlice()
        // Remove actions associated with the shapes
        activeSliceBG.removeAllActions()
        activeSliceFG.removeAllActions()
        
        // Set both slices shapes so they are visible
    ...
    func redrawActiveSlice() {
        ...
        // If more than 12 points, cycle through the find the 12 most recent
        while(activeSlicePoints.count > 12){
            activeSlicePoints.remove(at: 0)
        }
        
        // Starts line at first point and then go to the others in order
        ...
        // Update slice shape paths so they get drawn
    ...
    func createScore() {
        ...
        gameScore.position = CGPoint(x: 100, y: 100)
    }
    
    func createLives() {
        for i in 0 ..< 3 {
            let spriteNode = SKSpriteNode(imageNamed: "sliceLife")
            spriteNode.position = CGPoint(x: 974 - (i * 70) , y: 630)
            addChild(spriteNode)
